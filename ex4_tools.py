...
def find_threshold(D, X, y, sign, j):
    ...
    minimal_theta_loss = np.sum(D[y == sign])  # loss of the smallest
    # possible theta
    losses = np.append(minimal_theta_loss, minimal_theta_loss -
                       np.cumsum(D * (y * sign)))
...
def decision_boundaries(classifier, X, y, num_classifiers=1, weights=None):
    ...
    plt.xticks([])
    plt.yticks([])
    error = classifier.error(X, y, num_classifiers)
    plt.title('num classifiers = {}\nerror ratio = {}'.format(
        num_classifiers, error))
    plt.draw()
    return error
